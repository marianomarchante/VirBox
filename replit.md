...
*   **Events Module:** Manages company events with CRUD operations, date tracking, search functionality, and automatic notification modal at startup showing past/current-day unread events. Users can mark events as read (permanently hiding them from notifications) or delete them directly from the modal. Includes status filtering (All/Pending/Read) and text search with combined filtering capability.
...
*   **Menu Organization:** Main navigation excludes the "Empresas" (Companies) option; administrators access user management through the "Usuarios" menu item.
