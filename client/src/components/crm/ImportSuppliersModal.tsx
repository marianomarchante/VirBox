...
export function ImportSuppliersModal({ isOpen, onClose }: ImportSuppliersModalProps) {
  ...
  const { bulkCreateSuppliers } = useSuppliers();
    ...
    reader.onload = (e) => {
      try {
        ...
        const parsedSuppliers = jsonData.map((row: any) => {
          ...
          const name = findKey(["nombre", "name", "proveedor"]);
          ...
          const category = findKey(["categoria", "categoría", "category"]);
  ...
  const handleImport = () => {
    ...
    const validSuppliers = parsedData.filter(c => c.isValid);
    if (validSuppliers.length === 0) return;

    const toInsert = validSuppliers.map(c => ({
    ...
    bulkCreateSuppliers.mutate(toInsert, {
      onSuccess: () => {
        handleClose();
      }
    });
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Sube un archivo Excel (.xlsx, .xls) o CSV con tus proveedores.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            <div className="bg-muted/50 p-4 rounded-lg border text-sm">
              ...
              <ul className="list-disc pl-5 space-y-1 text-muted-foreground">
                ...
                <li><strong>Categoría</strong> (Opcional, por defecto "General")</li>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center bg-muted/50 p-3 rounded-md">
              <div className="flex items-center gap-2 text-green-600 dark:text-green-500">
                ...
                <span className="font-medium">{validCount} proveedores válidos</span>
              </div>
              {invalidCount > 0 && (
            ...
            <div className="bg-yellow-50 dark:bg-yellow-900/20 text-yellow-800 dark:text-yellow-200 p-3 rounded-md text-xs">
              <strong>Nota importante:</strong> Los proveedores cuyo NIF/CIF ya exista en la base de datos serán ignorados y no se actualizarán, evitando así duplicados.
            </div>
          </div>
        )}
        ...
        <DialogFooter className="mt-6">
          ...
          {parsedData.length > 0 && (
            <Button 
              ...
              disabled={validCount === 0 || bulkCreateSuppliers.isPending}
            >
              {bulkCreateSuppliers.isPending ? "Importando..." : `Importar ${validCount} proveedores`}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
